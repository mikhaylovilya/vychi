open Core
module QF = Vychi4lib.Quadrature_formulas

let print_ans_list ~theor_disc_flag (conf : QF.cqf_conf) =
  let open QF in
  let open Float in
  let meth_list =
    [ compound_left_rect_meth, Compound_Left_Rect_Method, 1. / 2., 0
    ; compound_right_rect_meth, Compound_Right_Rect_Method, 1. / 2., 0
    ; compound_middle_rect_meth, Compound_Mid_Rect_Method, 1. / 24., 1
    ; compound_trapezoid_meth, Compound_Trapezoid_Method, 1. / 12., 1
    ; compound_simpsons_meth, Compound_Simpsons_Method, 1. / 2880., 3
    ]
  in
  let () =
    Printf.printf
      "Функция : %s\nТочное значение интеграла = %f\n\n"
      conf.f_str
      (conf.integral conf.interval)
  in
  meth_list
  |> List.iter ~f:(fun (meth_f, c_meth, const, ast) ->
    let initial_result = conf.integral conf.interval in
    let result = meth_f conf in
    let abs_discrepancy = abs (initial_result - result) in
    let relative_discrepancy =
      match initial_result with
      | 0. -> nan
      | _ -> abs (initial_result - result) / abs initial_result
    in
    let ans = { c_meth; result; abs_discrepancy; relative_discrepancy } in
    let () = ans |> print_cqf_ans in
    if theor_disc_flag
    then (
      let a, b, m, h = conf.interval.left_b, conf.interval.right_b, conf.m, conf.h in
      let max =
        List.range 0 m
        |> List.map ~f:(fun i ->
          abs ((List.nth_exn conf.der_list ast) (a + (h * of_int i))))
        |> List.sort ~compare:Float.compare
        |> List.last_exn
      in
      let th_discrepancy = const * max * (b - a) * (h ** of_int Int.(ast + 1)) in
      Printf.printf "Теоритическая оценка погрешности = %f\n\n" th_discrepancy)
    else Printf.printf "\n\n")
;;

let eventloop func_list =
  let oc = Out_channel.stdout in
  let ic = In_channel.stdin in
  let print_prereq () =
    Printf.printf
      "Приближенное вычисление интеграла по составным квадратурным формулам 4.2\n\
       (Вариант 12)\n"
  in
  let print_s s =
    let () = Printf.fprintf oc "%s: " s in
    Out_channel.flush oc
  in
  let print_q q =
    let () = Printf.fprintf oc "%s?: " q in
    Out_channel.flush oc
  in
  let () = print_prereq () in
  let rec loop () =
    let () = print_s "Введите границу интегрироваания (a)" in
    let a = In_channel.input_line_exn ic |> Float.of_string in
    (*  *)
    let () = print_s "Введите границу интегрирования (b)" in
    let b = In_channel.input_line_exn ic |> Float.of_string in
    (*  *)
    let () = print_s "Введите число промежутков деления [a, b] (m)" in
    let m = In_channel.input_line_exn ic |> Int.of_string in
    let h = (b -. a) /. Float.of_int m in
    let () = Printf.printf "\n" in
    (*  *)
    let interval : QF.interval = { left_b = a; right_b = b } in
    let () =
      func_list
      |> List.iteri ~f:(fun i (f, integral, f_str, der_list) ->
        let (conf : QF.cqf_conf) =
          { f; f_str; integral; der_list; interval; m; l = 0; h }
        in
        if Int.( = ) i 4
        then print_ans_list ~theor_disc_flag:true conf
        else print_ans_list ~theor_disc_flag:false conf)
    in
    (* let _ = m, interval, func_list, print_ans_list in *)
    let () = print_q "Продолжить(y/n)" in
    match In_channel.input_line_exn ic with
    | "no" | "No" | "n" -> ()
    | "yes" | "Yes" | "y" -> loop ()
    | _ -> ()
  in
  loop ()
;;

let () =
  let open Float in
  let open QF in
  (* let df x = (-1. * sin x) - 2. in *)
  let f x = (3. * sin x) - (0.5 * (x ** 2.)) in
  let integral_f interval =
    let a, b = interval.left_b, interval.right_b in
    1. / 6. * ((18. * cos a) - (18. * cos b) + (a * a * a) - (b * b * b))
  in
  let f_0 x =
    let _ = x in
    1.
  in
  let integral_f_0 interval =
    let a, b = interval.left_b, interval.right_b in
    b - a
  in
  let f_1 x = 2. * x in
  let integral_f_1 interval =
    let a, b = interval.left_b, interval.right_b in
    (b * b) - (a * a)
  in
  let f_2 x = 3. * (x ** 2.) in
  let integral_f_2 interval =
    let a, b = interval.left_b, interval.right_b in
    (b ** 3.) - (a ** 3.)
  in
  let f_3 x = 4. * (x ** 3.) in
  let integral_f_3 interval =
    let a, b = interval.left_b, interval.right_b in
    (b ** 4.) - (a ** 4.)
  in
  let der_f_3 x = 12. * (x ** 2.) in
  let der_2_f_3 x = 24. * x in
  let der_3_f_3 x =
    let _ = x in
    24.
  in
  let der_4_f_3 x =
    let _ = x in
    0.
  in
  let func_list =
    [ f, integral_f, "f(x) = 3sin(x) - 1/2 * x^2", []
    ; f_0, integral_f_0, "f(x) = 1", []
    ; f_1, integral_f_1, "f(x) = 2 * x", []
    ; f_2, integral_f_2, "f(x) = 3 * (x ** 2)", []
    ; ( f_3
      , integral_f_3
      , "f(x) = 4 * (x ** 3)"
      , [ der_f_3; der_2_f_3; der_3_f_3; der_4_f_3 ] )
    ]
  in
  eventloop func_list
;;
