...
let print_ans_list (conf : QF.cqf_conf) =
  ...
  |> List.iter ~f:(fun (meth_f, c_meth, const, ast) ->
    ...
    Printf.printf
      "Уточненное значение по принципу Рунге = %f\n\
       Абсолютная погрешность для уточненного значения Рунге = %f\n\n"
      runge_result
...
let eventloop func_list =
  ...
  let print_prereq () =
    Printf.printf
      "Приближенное вычисление интеграла по составным квадратурным формулам 4.3\n\
       (Вариант 12)\n"
  in
  let print_s s =
  ...
  let rec loop () =
    ...
    (*  *)
    let () = print_s "Введите границу интегрироваания (b)" in
    let b = In_channel.input_line_exn ic |> Float.of_string in
    (*  *)
    let () = print_s "Введите параметр (m)" in
    let m = In_channel.input_line_exn ic |> Int.of_string in
    let () = print_s "Введите параметр (l)" in
